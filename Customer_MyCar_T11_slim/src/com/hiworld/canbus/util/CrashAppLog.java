...
abstract class CrashAppLog implements UncaughtExceptionHandler{
	public static String TAG = "CrashAppLog";
	
	/**
	 * 默认放在内存卡的root路径
	 */
	private String CAHCE_CRASH_LOG = Environment.getExternalStorageDirectory()
    ...
    //用来存储设备信息和异常信息  
    private Map<String, String> crashAppLog = new HashMap<String, String>();  
	...
	public void uncaughtException(Thread thread, Throwable throwable) {
		...
		try {
			...
			/**
			 * hanlderException此方法是是否处理异常日志，如果处理那么就返回tru *
			 * e，如果处理遇到问题那么就返回false，交由系统默认处理
			 **/
			if (!hanlderException(throwable)
					&& mUncaughtExceptionHandler != null) {

				/**
				 * 如果此异常不处理则由系统自己处理
				 */
				this.mUncaughtExceptionHandler.uncaughtException(thread,
						throwable);
			...
			if (fields != null && fields.length > 0) {
				...
				for (Field field : fields) {
					...
					if (field != null) {
						...
						crashAppLog.put(field.getName(), field.get(null)
								.toString());
					}
				}
			}
	...
	private void writerToFile(String s) {
		...
		try {
			 ...
			 * 创建日志文件
			 */
			File file = new File(CAHCE_CRASH_LOG + "/"  + fileName);
