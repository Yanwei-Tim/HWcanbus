...
public class GaoDeReceiver extends BroadcastReceiver {
	/**
	 * 导航类型，对应的值为int类型<br>
	 * 0：GPS导航<br>
	 * 1：模拟导航
	...
	/**
	 * 当前道路类型，对应的值为int类型 0：高速公路 1：国道 2：省道 3：县道 4：乡公路 5：县乡村内部道路 6：主要大街、城市快速道 7：主要道路
	 * 8：次要道路 9：普通道路 10：非导航道路
	 */
	...
	private static GaoDeReceiver OBJ = new GaoDeReceiver();
	
	public static GaoDeReceiver getOBJ() {
		return OBJ;
	} 
	
	public void reg(Context c) { 
		IntentFilter filter = new IntentFilter(ACTION_GAODE_INFO);
		c.registerReceiver(this, filter);
		LogsUtils.i("reg limitSpeed " );
	}
	...
	public void onReceive(Context context, Intent intent) {
		...
		if (action.equals(ACTION_GAODE_INFO)) {
			int KEY_TYPE = intent.getIntExtra("KEY_TYPE", 0);
			switch (KEY_TYPE) {
			case 10001://
				int limitSpeed = intent.getIntExtra(CAMERA_SPEED, -1); 
				if(DataCanbus.sLimiteSpeed != limitSpeed) {
					DataCanbus.sLimiteSpeed  = limitSpeed;
					DataCanbus.NOTIFY_EVENTS[FinalOBD.U_LIMIT_SPEED].onNotify(); 
				}
				LogsUtils.i("limit " + limitSpeed);
				break;
			default:
				break;
			}
