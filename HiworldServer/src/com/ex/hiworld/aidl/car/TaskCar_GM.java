...
public class TaskCar_GM extends BaseCar {
	...
	int KeyCanKeyTable[][]=
	 {
	 			...
	 			{0x01,		FinalKeyCode.KEY_BLACK_SCREEN},	
	 			...
	 			{0x08,		FinalKeyCode.KEY_CODE_DVD},	
	 			...
	 			{0x10,		FinalKeyCode.KEY_CODE_PLAY},	
	 			{0x11,		FinalKeyCode.KEY_CODE_MUTE},	
	 			{0x12,		FinalKeyCode.KEY_CODE_NULL},	
	 			...
	 			{0x21,		FinalKeyCode.KEY_CODE_NULL},	
	 			{0x22,		FinalKeyCode.KEY_CODE_NULL},	
	 			{0x23,		FinalKeyCode.KEY_CODE_NULL},	
	 			{0x24,		FinalKeyCode.KEY_CODE_MODE},	
	 			...
	 			{0x2b,		FinalKeyCode.KEY_CODE_CARSETTTING},	
	 			{0x2c,		FinalKeyCode.KEY_CODE_MODE},	
	 			{0x2d,		FinalKeyCode.KEY_CODE_CARSETTTING},	
...
//			}
				...
				CanKey = (data[start+4]&0xFF)|0x80;
				CanInfos.onKeyEvent(KeyCanKeyTable,  CanKey, data[start + 5]);
...
//			ModuleCallbackList.update(DataCanbus.MCLS, U_AIR_SHOW, new int[]{B0>>7&0x01}, null, null);
			...
			switch (value) {
				case 0xFE:
				case 0xFF:{
					HandlerTaskCanbus.update(U_AIR_TEMP_LEFT, 	value);
					break;
				}
				default: {
			...
			switch (value) {
				case 0xFE: 
				case 0xFF:{
					HandlerTaskCanbus.update(U_AIR_TEMP_RIGHT, 	value);
					break;
				}
				default: {
...
//				break;
	...
	private void updatePairingBtPassword(final int[] password, int start, int len) {
		...
		if(!str.equals("")){
//			if (HandlerTaskCanbus.update(U_ANJIXING_BT_PASSWORD, str, mBtPassword)) {
//				mBtPassword = str;
//			}
		}
	}
	...
	private void updateBtPhoneName(final int[] name, int start, int len) {
		...
		final String str = buffer.toString();
		if(!str.equals("")){
//			if (HandlerCanbus.update(U_ANJIXING_BT_NUMBER, str, mBtNumber)) {
//				mBtNumber = str;
//			}
		}
	}
	...
	{	
		...
		if(data >= 226)		//2260xe2
			...
			{
			t1 = (data * 10) / 255;  //将数值转换成 0-10
			}
		return t1;
	}
	
	static int knob_right = 0;
	static int knob_left = 0;
	static int knob_steer_right = 0;
	static int knob_middle = 0;
	
//	static void Knob_Scan_Vcan(int data)
//	{
//		
//		if(data!=knob_left)
//		{
//			if(data!=knob_left)
//			{
//				if((knob_left>=0x80 && knob_left<=0xff)&& (data>=0 && data<0x80))//从0xff，到0
//				{
//					knob_left=data;
//					if(DataMain.sAppId==FinalMain.APP_ID_RADIO)
//						HandlerMain.mcuKeyUp();
//					else
//						HandlerMain.mcuKeyRight();
//					return;
//				}
//				if(( data>=0x80&&data<=0xff)&& (knob_left>=0 && knob_left<0x80))//从0到0xff
//				{
//					knob_left=data;
//					if(DataMain.sAppId==FinalMain.APP_ID_RADIO)
//						HandlerMain.mcuKeyDown();
...
//	static void Knob1_Scan_Vcan(int data1,int data2)
//	{
//		
//		if(data1!=knob_right||data2!=knob_steer_right)
//		{
//			if(data1!=knob_right)
//			{
//				if((knob_right>=0x80 && knob_right<=0xff)&& (data1>=0 && data1<0x80))//从0xff，到0
//				{
//					knob_right=data1;
//					
...
//					return;
//				}
//				if((data1>=0x80&&data1<=0xff)&& (knob_right>=0 && knob_right<0x80))//从0到0xff
//				{
//					knob_right=data1;
//					
...
//				}
//			}
//			else if(data2!=knob_steer_right)
//			{
//				
//				if((knob_steer_right>=0x80 && knob_steer_right<=0xff)&& (data2>=0 && data2<0x80))//从0xff，到0
//				{
//					knob_steer_right=data2;
//					if(DataMain.sAppId==FinalMain.APP_ID_RADIO)
//						HandlerMain.mcuKeyUp();
//					else
//						HandlerMain.mcuKeyRight();
//					return;
//				}
//				if(( data2>=0x80&&data2<=0xff)&& (knob_steer_right>=0 && knob_steer_right<0x80))//从0到0xff
//				{
//					knob_steer_right=data2;
//					if(DataMain.sAppId==FinalMain.APP_ID_RADIO)
//						HandlerMain.mcuKeyDown();
...
//	static void Knob2_Scan_Vcan(int data)
//	{
//		if(data!=knob_middle)
//		{
//			if((knob_middle>=0x80 && knob_middle<=0xff)&& (data>=0 && data<0x80))//从0xff，到0
//			{
//				knob_middle=data;
//				HandlerMain.mcuKeyRight();	
//				return;
//			}
//			if(( data>=0x80&&data<=0xff)&& (knob_middle>=0 && knob_middle<0x80))//从0到0xff
//			{
//				knob_middle=data;
//				 HandlerMain.mcuKeyLeft();
//				return;
...
//				/*//没找到在哪里哦
//				 * StatusApp.setCarPhoneWork(state);*/
//				DataMain.sCmd.appId(sChannelPre);
//			}
//			isWork = 0;
//			onStartState = 0;
//			break;
...
//		if (HandlerCanbus.update(U_ANJIXING_NUMBER, num, mNumber)) {
//			mNumber = num;
//		}
	}
	private String mNumber;
	
	private void updateOnStarPhoneNumber(final int[] data, int start, int len) {
//		StringBuffer buffer = new StringBuffer();
//		for (int i = 0; i < len; i++) {
//			if (number[start + i] == '\0')
//				break;
...
//		}
//		String num = buffer.toString();
//		if (HandlerCanbus.update(U_ANJIXING_PHONE_NUMBER, num, mPhoneNumber)) {
//			mPhoneNumber = num;
//		}
	}
...
//			if (data_tts[start + i] == '\0'&&data_tts[start + i] == '\0'&& data_tts[start + i] == '\0'){
//				length = i+3;
//				TTS_END = true;
//				break;
//			}
...
//				stopVoice();
//				HandlerRemove.getInstance().postDelayed(new Runnable() {
//					@Override
//					public void run() {
...
//				}, 1500);
//				
//				if (HandlerCanbus.update(U_ANJIXING_TTS,Str_TTS , Str_TTS_P)) {
//					Str_TTS_P = Str_TTS;
//				}
//			}
//			Str_TTS = null;
//			TTS_END = false;
//			ToolkitDev.writeMcu(0xE3, (byte)0x02, 0xBA,  0x06, 0x00);
//		}
	}
...
//	private void updateOnStarState(int state) {
//		HandlerCanbus.update(U_ANJIXING_STATE,		    state);
//		onStartState = state;
//		switch (state) {
//		case 1:
//		case 2:
