...
public class MyLinkHostServer extends Service {
    ...
    public void onDestroy() {
        super.onDestroy(); 
        LogsUtils.i(getClass().getSimpleName() + " onDestroy");
    ...
    public void onConnected(ConnEvent event) {
        if ("conn".equals(event.type) && event.success) {//服务链接成共
            ...
            LogsUtils.i("LinkHostCar onConnected");
            PrintScreenView.getMsgView().msg(" LinkHostCar onConnected.. ");  

            observableServer();
            // 对上
            HandlerUI.getInstance().post(new ConnectClientServer());
        }
    }
    ...
    public void onMessageHandle(Message msg) {
        if (msg.module == FinalSyuModule.MODULE_CODE_MAIN) {
            switch (msg.code) {
                ...
                case FinalMain.U_APP_ID:
                    if (DataHost.sAppid != msg.ints[0]) {
                        ...
                        updateAppInfo();
                    	LogsUtils.i(" cur app id:" + DataHost.sAppid);
            ...
            switch (msg.code) {
				...
				if (msg.ints == null)
					break;
				if (msg.ints.length > 0 && msg.ints.length < 3) {
					if (DataCanbus.isEnterUpdateMode) {
						LogsUtils.i("isEnterUpdateMode: " + LogsUtils.toHexString(msg.ints));
						HandlerTaskCanbus.parseCanbusData(msg.ints);
					}
				} else
	...
	private void parseData(int[] data) {
		if (data[3] == 0xF0) {
			...
			if (str != null && !DataCanbus.sVersionCanbox.equals(str)) {
				...
				DataCanbus.canbusType = DataCanbus.canbusId & 0xFFFF;
				DataCanbus.canbusLevel = DataCanbus.canbusId >> 16 & 0xFFFF;
