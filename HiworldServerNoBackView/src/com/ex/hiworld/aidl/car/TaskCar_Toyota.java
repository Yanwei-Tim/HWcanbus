...
public class TaskCar_Toyota extends BaseCar {
	...
	private static final int U_CAR_TYPE 					= U_AMP_VOLUME + 8;
	
	public static final int U_CNT_MAX						= U_CAR_TYPE+1;
...
//			}	
			...
			for (int i = 0; i < TRIPOILEXPENDMAX; i++) {
				int val = ((data[start + 2 * i + 4] & 0xff) << 8) | (data[start + 2 * i + 5] & 0xff) | unitOilExpand;
				if (mTripoilexpend[i] != val) {
					HandlerTaskCanbus.update(U_TRIP_OIL_EXPEND, new int[] { i, val });
					mTripoilexpend[i] = val;
				}
			}	
			break;
		}
		case 0x17: { // 油耗/里程信息3 记录
			...
			for (int i = 0; i < MINUTEOILEXPENDMAX; i++) {
				int val = ((data[start + 2 * i + 2] & 0xff) << 8) | (data[start + 2 * i + 3] & 0xff) | unitOilExpand;
				if (mMinuteoilexpend[i] != val) {
					mMinuteoilexpend[i] = val;
					HandlerTaskCanbus.update(U_LAST_N_MINUTE_OIL_EXPEND, new int[] { i, val });
				}
			}
			break;
...
//		LogsUtils.i("in S_CAR_TYPE...  " + spUtils.get(S_CAR_TYPE, 0));
	...
	private void setCarType(int i, boolean update) {
		if (mCarOffset != i) {
			int canbusoffset = 0;
			switch (i) {
			case 1: // 霸道
			...
			spUtils.set(S_CAR_TYPE, i);
			HandlerTaskCanbus.update(U_CAR_TYPE, i);
			mCarOffset = canbusoffset;

			SendFunc.send2Canbus(0x2D, new int[] { 1, i });
			
			if(update) {
				updateCanbusIdOffSet(i);
			}
		}
	}
...
//		case FinalBt.PHONE_STATE_PAIR:
	...
	private Rect[] btnRects = new Rect[] {
			new Rect(0, 800, 256, 1000),
			new Rect(256, 800, 512, 1000),
			new Rect(512, 800, 768, 1000),
			new Rect(768, 800, 1024, 1000),
	};
	...
	public void cmd(int cmd, int[] ints, float[] floats, String[] strings) throws android.os.RemoteException {
		...
		case FinalCanbus.C_HW_SCREEN_TOUCH: {
			if (ints != null && ints.length == 2) {
				int touch_x = ints[0] * 1023 / MyApp.getWidth();
				int touch_y = ints[1] * 1000 / MyApp.getHeight();

				LogsUtils.i("C_HW_SCREEN_TOUCH " + ints[0] + " " + ints[1] + " / " + touch_x + " " + touch_y);
				SendFunc.send2Canbus(0x2C,
						new int[] { 0x01, touch_x >> 8, touch_x & 0xff, touch_y >> 8, touch_y & 0xff, 0 });
				for (int i = 0; i < btnRects.length; i++) {
					if (touch_y > btnRects[i].top) {
						if (touch_x > btnRects[i].left && touch_x < btnRects[i].right) {
							int index = i + 1;
							SendFunc.send2Canbus(0xFA, new int[] { index, 1 });
							SendFunc.send2Canbus(0xFA, new int[] { index, 0 });
							LogsUtils.i("C_HW_SCREEN_TOUCH 0xFA  " + index);
							break;
						}
					}
				}
	...
	public void registerCallback(ITaskCallback cb, int code) throws RemoteException {
		if (code >= 0 && code < U_CNT_MAX) {
			switch (code) {
			case U_TRIP_OIL_EXPEND: {
				for (int j = 0; j < mTripoilexpend.length; j++) {
					if(mTripoilexpend[j] != 0)
						HandlerTaskCanbus.update(U_TRIP_OIL_EXPEND, new int[] { j, mTripoilexpend[j] });
				}
				break;
			}
			case U_LAST_N_MINUTE_OIL_EXPEND:
				for (int j = 0; j < mMinuteoilexpend.length; j++) {
					if(mMinuteoilexpend[j] != 0)
						HandlerTaskCanbus.update(U_LAST_N_MINUTE_OIL_EXPEND, new int[] { j, mMinuteoilexpend[j] });
				}
				break;
			default:
				if(DataCanbus.DATA[code] != 0)
					HandlerTaskCanbus.update(code, DataCanbus.DATA[code]);
				break;
			}
		}
	}
