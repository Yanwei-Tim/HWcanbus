...
public class TaskCar_VW extends BaseCar {
	...
	public static final int U_AIR_OUT_TEMP                     			= U_CARINFO_BEGIN + 29; 
	private static final int U_SET_49D0               			= U_AIR_OUT_TEMP + 1; 	// 胎压保存状态
	private static final int U_SET_49D17               			= U_SET_49D0 + 1;  		// 报警级别
	private static final int U_SET_49D13               			= U_SET_49D0 + 2;   	// 右后轮
	private static final int U_SET_49D12               			= U_SET_49D0 + 3;  		// 左后轮
	private static final int U_SET_49D11               			= U_SET_49D0 + 4;  		// 右前轮
	private static final int U_SET_49D10               			= U_SET_49D0 + 5;  		// 左前轮
	private static final int U_SET_49D20               			= U_SET_49D0 + 6;  		// 胎压报警使能
	
    public static final int U_CNT_MAX 									= U_SET_49D20 + 5;
	
    

    public static final int C_SET		= 1;
    public static final int C_SET_TIRE		= 2;
	...
	public void onHandler(int[] data) {
		...
		case 0x49: { // tire
			...
			byte B2 = (byte) data[start + 4];
			byte B3 = (byte) data[start + 5];
			HandlerTaskCanbus.update(U_SET_49D0, B0 & 0x01);
			HandlerTaskCanbus.update(U_SET_49D17, B1>>7 & 0x01);
			HandlerTaskCanbus.update(U_SET_49D13, B1>>3 & 0x01);
			HandlerTaskCanbus.update(U_SET_49D12, B1>>2 & 0x01);
			HandlerTaskCanbus.update(U_SET_49D11, B1>>1 & 0x01);
			HandlerTaskCanbus.update(U_SET_49D10, B1>>0 & 0x01);
			HandlerTaskCanbus.update(U_SET_49D20, B2 & 0x0F);
			break;
		}
		case 0x72: {
			byte B0 = (byte) data[start + 2];
		...
		case 0x73: {
			...
			HandlerTaskCanbus.update(U_AIR_WIND_LEVEL_RIGHT,  B5 & 0x0F);
             ...
             SendFunc.sendOutTemp(1, tempature); 
           	...
           	HandlerTaskCanbus.update(U_AIR_OUT_TEMP, B6&0xFF);
           	HandlerTaskCanbus.update(U_DOOR_FR,  B7>>7 & 0x01);
 			HandlerTaskCanbus.update(U_DOOR_FL,  B7>>6 & 0x01);
	...
	public void cmd(int cmd, int[] ints, float[] floats, String[] strings) throws android.os.RemoteException {
         ...
         case C_SET_TIRE:{
             if (ints != null && ints.length > 0) {
                 SendFunc.send2Canbus(0x4D, ints);
             }
        	 break;
         }
		 }
